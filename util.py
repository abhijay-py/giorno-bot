...
class Utility(commands.Cog):
    ...
    @commands.command()
    async def latex(self, ctx, *, expression: str = ""):
        if not await pluginEnabled("utility", ctx.guild):
            return
        if expression == "":
            await ctx.send("Please enter a valid latex expression.")

        lat = sympy.latex(expression)
        ...
        plt.savefig("./tempLatex/" + str(ctx.author.id) + ".png", bbox_inches='tight', dpi=100, transparent=True)
        with open("./tempLatex/" + str(ctx.author.id) + ".png", "rb") as fh:
            file = discord.File(fh, filename="./tempLatex/" + str(ctx.author.id) + ".png")
            await ctx.send(file=file)
        await asyncio.sleep(2)
        os.remove("./tempLatex/" + str(ctx.author.id) + ".png")
